...
//     function getBytes(string json, Token t) constant internal returns (string) {
//         bytes memory s = bytes(json);
//         bytes memory result = new bytes(t.end-t.start);
//         for (uint i=t.start; i<t.end; i++) {
//             result[i-t.start] = s[i];
//         }
